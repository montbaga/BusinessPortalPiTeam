...
WebUI.callTestCase(findTestCase('db_commit'), [:], FailureHandling.STOP_ON_FAILURE)
	...
	WebUI.setText(findTestObject('Object Repository/OUTAGESINUAT/input_Search_txtAccountId'), '900156777')
	...
	println(CalviRole)
		...
		if (CalviRole.contains("Partner")) {
			println("Calvi Flow for Partner is passed in UI :) ")
		  } else {
			println("Calvi Flow for Partner is not passed in UI)  :( ")
		  }
...
println("Calvi Flow for Partner is passed in Database")
